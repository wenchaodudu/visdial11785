...
class Dataset(data.Dataset):
    ...
    def __init__(self, data_file, img_file, train=True):
        ...
        self.answers = np.zeros((self.length, 10, 22), dtype=np.int32)
    ...
    def __getitem__(self, index):
        return self.images[index], self.captions[index], self.questions[index], \
               ...
               self.cap_len[index], self.ques_len[index], self.ans_len[index], \
               self.opt_len[index]
...
def collate_fn(data):
    ...
    img_seqs, cap_seqs, ques_seqs, ans_seqs, opt_seqs, ans_idx_seqs, cap_lens, ques_lens, ans_lens, opt_lens = zip(*data)
    return img_seqs, cap_seqs, ques_seqs, ans_seqs, opt_seqs, ans_idx_seqs, cap_lens, ques_lens, ans_lens, opt_lens
